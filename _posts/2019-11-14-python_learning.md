...
- deque：list的访问很快，但是插入和删除很慢，deque是双向列表，能使得插入和删除都变得高效，栈和队列的功能都可以实现，因为它除了pop和append之外，还支持appendleft和popleft
...
是进程和线程的升级版，进程和线程都面临着内核态和用户态的切换问题，协程就是用户自己控制切换的时机，不需要陷入内核态，比如yield语句思想
...
两者的区别在于在存在类的继承的情况下对多态的支持不同，classmethod需要实力化之后的对象调用，staticmethod用类名就可以调用

能用classmethod的时候就使用classmethod，对类的用户可见的功能使用classmethod，对类的用户不可见的功能可以考虑使用metaclass
...
python中有个概念叫时间元组struct\_time，包含九个元素（年份，月，日，小时，分，秒，星期几，一年的第几天，夏令标识）, localtime方法可以获取当前的时间元组

```python
strftime("%Y-%m-%d %H:%M:%S", time.localtime())
可以把时间元组转化成制定的标准字符串格式

strptime( s, "%Y-%m-%d %H:%M:%S")
可以把格式字符串转化为时间元组形式
```

time方法可以获取当前时间的时间戳，时间戳是指格林威治时间自1970年1月1日（00:00:00 GMT）至当前时间的总秒数
