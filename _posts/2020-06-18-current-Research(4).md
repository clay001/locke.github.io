...
区别是CV用了交叉验证来选择正则化系数C，class\_weight可以选balanced。solver参数是损失函数的优化方法，liblinear用坐标轴下降的方法优化，lbfgs是拟牛顿法的一种，newton-cg是牛顿法，sag是随机平均梯度下降

## 数据预处理

数字如果比较大，或者不是正态分布的时候，为了让结果更好收敛。我们可以做标准化和归一化操作

min-max标准化就是01标准化，z-score标准化就是标准差标准化。一般来说在涉及距离度量相似性的时候z-score的表现更好一些。还有一些其他的处理方法在preprocessing包里都有。

## 多分类的AUC

两种方法分别从macro和micro，也就是类别和个体的角度去转化成二分类的case

## 视频编解码原理

源视频数据量很大，在分辨率上还要考虑位色，秒帧等等，对存储空间的带宽的要求都很高。所以需要进行编码，从第一代的MPEG-1/2 到 第二代H.264/AVS 到 第三代H.265/AVS2，压缩的力度越来越大。

帧内压缩可以用空间冗余的思路来进行替代

帧间压缩可以用时间冗余的思路来进行替代

上述的两种方法会压缩成一串字符串，继续进行压缩就属于编码冗余的范畴

编码的核心技术步骤为预测，变换，量化，熵编码，滤波

预测分为帧内预测和帧间预测。I帧是关键帧，一般只做帧内预测，根据前一幅图像预测得到的图像叫P帧，根据前后双向预测得到的叫做B帧。根据I帧和一系列B帧，P帧组成的一组图像叫做GOP

变换指的是用比如离散余弦DCT，小波变换，把图像从空域变换到频域上，进行队形调整

量化是对变换后的矩阵通过一个步长进行分级，这样高频的右下角信息就会变成0，然后对数值进行Z字形扫描得到数字

熵编码是先行程编码把连续重复的数据用重复的次数表示，然后进行变长编码（霍夫曼编码）进一步减少原始数据长度

## 多分类任务

随机森林: 一种基于树和bagging方法的集成分类器，该分类器不需要缩放和去噪的处理

逻辑回归分类器：受变量范围的影响，且容易收到多重共线性的影响。耗时会略长，要减少时间可以结合PCA来做

主成分分析：把低级变量转换为更高维的空间，从而减少所需变量的数量

ANN人工神经网络：非线性，具有自动特征工程和降维的功能。耗时略长

线性支持向量机SVM：耗时少，准确率也还不错

XGBoost：可以自动减少特征集，效果很好，但耗时很长

## 异常检测

还有另外两个概念，噪声消除指的是从有意义的信号中消除噪音，新颖性检测是识别出未观察到的模式

统计模型下的异常是对数据分布作出假设，然后用距离均值作为离群判断

线性模型与非线性模型是找出在低维嵌入表现不好的数据，因为正常点的重建误差应该小于异常点

相似度衡量模型是通过观察数据之间的邻近程度来区分

集成与模型融合：异常检测是无监督的，所以基本是通过bagging的方式集成的

## 数据不平衡问题

扩大数据集，其他评价指标，数据重采样，尝试产生人工数据样本，不同的分类算法，对模型进行权重惩罚，转换思路（转为异常检测）

更换损失函数，Focal Loss在原来带权值的交叉熵函数上进行改进，

## 特征选择

去掉取值变化小的特征，单变量特征选择（卡方检验），皮尔森相关系数，互信息和最大信息系数，距离相关系数，基于学习模型的特征排序，线性模型加正则化（L1是Lasso，迫使模型变稀疏；L2是Ridge，迫使系数变平均），随机森林（平均不纯度减少，平均精确率减少）

卡方检验是用方差来衡量某个观测频率和理论频率之间差异的算法，皮尔森检验就是一种卡方检验的方法，用方差和的方式计算卡方值，算出自由度，比较卡方值和对应自由度的卡方分布，判断显著性。p value是假设检验的统计量，根据卡方值和自由度可以计算得到。零假设指的是两者之间独立。
